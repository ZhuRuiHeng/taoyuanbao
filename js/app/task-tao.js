...
//				$('.xiaoijiage').html(data.xuqiu_jiage);
	...
	function upload(){
	    ...
	    var  task = plus.uploader.createUpload(apiRoot + '?m=Home&c=Photo&a=uploadOnePhoto',{ method:"POST"},function ( t, status ) { 
			...
			if (status == 200 ) {
//				toast( "头像上传中" );
				plus.storage.setItem('tao',t.responseText);  //  获取到的图片路径 
...
//		zuorenwuqian = zongrentou*xuqiu_jiage+zhubaob_liul*xuyao_ren*xuqiu_jiage; 
	...
	$('.mui-btn-block').on('tap',function(){ 
		// 照片路径
		var pic_path = plus.storage.getItem('tao');
...
////////////////// 判断前面几项是否为空  ///////////////////////////////////////////////
		...
		if(!taokouling){ toast('未填写如何找到宝贝');return; }
///////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////	        
        ...
        $.ajax({
        	type: "get",
        	url : apiRoot + "?m=Home&c=Release&a=add_taokouling", 
        	data : {
        		userid : userid,
...
////////////////////// 判断输入的任务值是否大于0 且不是负数   ///////////////////////////////////////////////////////////////////////////////////////////////////
           //// 判断输入的任务值是否大于0 且不是负数 且每一项都不能大于需要任务人数
//      var zhi;
//      $("input[type='text'][class='gaibian']").each(function(){			
//			zhi =$(this).val();
////			console.log(zhi);
//			pan_zhi(Number(zhi),xuyao_ren);
//		})
   //// 判断输入的任务值是否大于0 且不是负数
