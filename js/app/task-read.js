...
//				$('.xiaoijiage').html(data.xuqiu_jiage);
				...
				console.log(xuqiu_jiage);  
	            console.log(pingtai_bilv);
			}else{ 
	...
	function upload(){
	    ...
	    var  task = plus.uploader.createUpload(apiRoot + '?m=Home&c=Photo&a=uploadOnePhoto',{ method:"POST"},function ( t, status ) { 
			...
			if (status == 200 ) {
//				toast( "头像上传中" );
				plus.storage.setItem('read',t.responseText);  //  获取到的图片路径 
...
//		zuorenwuqian = zongrentou*xuqiu_jiage+zhubaob_liul*xuyao_ren*xuqiu_jiage;
	...
	$('.mui-btn-block').on('tap',function(){ 
		// 照片路径
		var pic_path = plus.storage.getItem('read');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////	        
        ...
        $.ajax({
        	type: "get",
        	url : apiRoot + "?m=Home&c=Release&a=add_liulan",
        	data : {
        		userid : userid,
